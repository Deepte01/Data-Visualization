...
// const colours=d3.scaleOrdinal(['#DB4437','#4285F4','#FF8800','#0F9D58','#B8009F','#FFD500','#EBE652']);
...
.html(d=>{
    ...
    content += `<div class='content'>CC : ${d.data.info.CC}  ${Math.round(d.x)}</div>`
    content += `<div class ='moreInfo content' >Click to see more Info</div>`
...
.attr('transform',`translate(${innerRadius+300},-380)`)
...
const handleRightClick = (data,index,el)=>{
  ...
  paperAddInfo.push(data)
  
  paperAddInfo1=Array.from(new Set(paperAddInfo))
    ...
    .text("Year: "+d.data.info.year)
      ...
      importNodes.forEach((d1,i,n)=>{
       ...
       console.log('parent_root',total)
            ...
            fetchedNode.push(total.filter((el)=> {
              // console.log('inside nodes:',name);
                  var flag=false;
                  if(el.data.name === d1)
                  {
